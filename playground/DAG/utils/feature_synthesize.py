...
def weights_calculate(tasks):
    weight_tasks = {}
    for task in tasks:
        feature = task_features(task.job)
        weight = feature[task.task_index]['first_layer_task'] + feature[task.task_index]['first_layer_instance'] \
                 + feature[task.task_index]['layers_task'] + feature[task.task_index]['child_task_numbers'] \
                 + feature[task.task_index]['child_instance_numbers']
